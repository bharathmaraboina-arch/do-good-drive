...
import { ArrowLeft, CheckCircle2, Sparkles, FileText } from 'lucide-react';
...
export default function NewPostPage() {
  ...
  const onSubmit = async (data: PostFormData) => {
    createPost({
      ngoProfileId: profile?.id === 'ngo-demo-1' ? 'ngo-1' : profile?.id || 'ngo-1',
      ngoName: profile?.organizationName || 'GreenCanopy Initiative',
      type: data.type,
      title: data.title,
      body: data.body,
  ...
  return (
    <AppShell>
      <div className="max-w-3xl">
        ...
        <PageHeader
          title="Create Community Dispatch"
          description="Author an update, field report, or verified impact story to broadcast across the marketplace."
        />
        ...
        {isSuccess ? (
          <div className="bg-white p-8 rounded-xl border border-[#E8E3E8] shadow-xs text-center">
            ...
            <h2 className="text-lg font-bold text-[#25232A]">Dispatch Created Successfully!</h2>
            <p className="text-xs text-[#8B8790] mt-1">
              Your post has been processed and updated on the community home feed.
            </p>
          </div>
        ) : (
          <form
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                placeholder={
                  currentType === 'IMPACT_STORY'
                    ? 'e.g. Riparian Corridor Revitalization: 1,400 Native Shrubs Planted'
                    : 'e.g. Field Supply Update: First-Aid Kits and Tools Restocked'
                }
            ...
            {currentType === 'IMPACT_STORY' && (
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. 1,400 Native Shrubs Planted or 500 Families Assisted"
                  className="w-full px-3 py-2 text-sm text-[#25232A] placeholder-[#8B8790] bg-[#FBFAF8] border border-[#E8E3E8] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#A85AAA] focus:border-[#6D3A70]"
                />
              </div>
            )}
            ...
            <div>
              ...
              <textarea
                rows={5}
                {...register('body')}
                placeholder="Share the context, work completed, volunteer appreciation, or upcoming next steps..."
                className="w-full px-3 py-2 text-sm text-[#25232A] placeholder-[#8B8790] bg-[#FBFAF8] border border-[#E8E3E8] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#A85AAA] focus:border-[#6D3A70]"
              />
              {errors.body && (
            ...
            <div>
              ...
              <input
                type="text"
                {...register('imageUrl')}
                placeholder="https://example.org/photo-of-volunteers.jpg"
                className="w-full px-3 py-2 text-sm text-[#25232A] placeholder-[#8B8790] bg-[#FBFAF8] border border-[#E8E3E8] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#A85AAA] focus:border-[#6D3A70]"
              />
            </div>
            ...
            <div className="pt-4 border-t border-[#FAF5FA] flex flex-col sm:flex-row items-center justify-between gap-3">
              <label className="flex items-center gap-2 text-xs text-[#6B6870] cursor-pointer">
                ...
                />
                <span>Publish immediately to community feed</span>
              </label>
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {isSubmitting ? 'Saving...' : 'Save & Publish Dispatch'}
                </button>
              </div>
            </div>
          </form>
